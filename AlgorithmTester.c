...
int main(int argc, char * argv[]){
	...
	printf("Which integer sorting algorithm do you want to test?\n\n1 - Soares Counting Sort (My algorithm);\n2 - Quick Sort (stdlib.h implementation);\n3 - Insertion Sort;\n4 - Selection Sort;\n5 - Counting Sort (Doesn't work with negative integers);\n6 - Shell Sort;\n7 - Bubble Sort\n8 - Radix Sort(Doesn't work with negative integers);\n9 - Soares Counting Sort 2;\n10 - Soares Counting Sort(OPTIMIZED);\n0 - Exit Program;\n\nOption: ");
	scanf("%d", &select);
	if((select>10) || (select<1)){
		if(!select){
			...
			fprintf(stderr,"Menu Selection Error.");
		}
		exit(1);
	}
	else{
		...
		printf("\n\nAverage run time (%d trials): %f secs\nTotal run time: %f secs", trials, (t/trials), t);
	}
